...
var { VehicleMakeList, DeleteVehicleAPI, alldeleteUser } = require("../Api/ApiRoutes");
...
function CompanyNameList() {
  ...
  const deleteRecord = (e) => {
    ...
    // );
    // if (delete_property) {
      apiRequest(DeleteVehicleAPI.method, DeleteVehicleAPI.url + `${currentId}`)
        .then((response) => {
          let data = response.data;
          if (response.data.code == 200) {
            //setrecords(response.data.data);
  ...
  const handlePageChange = (pageNumber) => {
    ...
    apiRequest(VehicleMakeList.method, VehicleMakeList.url + `?page=${pageNumber}`)
      .then((response) => {
        let data = response.data;
        if (response.data.code == 200) {
          setrecords(response.data.data);
          ...
          <Header />
          {
            ...
            <div className="card card-shadow">
               ...
               <button
                          ...
                          className=""
                          exact
                          to={`/admin/edit-vehicle-company/${record.id}`}
                        >
