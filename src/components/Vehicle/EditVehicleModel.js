...
var {EditModelAPI,UpdateModelAPI, VehicleMakeList,VehicleModelList} = require('../Api/ApiRoutes');
...
function EditVehicleModel(props) {
  ...
  React.useEffect(()=>{
    ...
    apiRequest(EditModelAPI.method, EditModelAPI.url + id)
      .then((response) => {
  ...
  const handleSubmit = (values) => {
    ...
    apiRequest(UpdateModelAPI.method, UpdateModelAPI.url + id, form_data)
      .then((response) => {
        if (response.data.code == 200) {
          let data = response.data.data;
...
console.log('vehiclemodelllll',record);
  return (
    <React.Fragment>
