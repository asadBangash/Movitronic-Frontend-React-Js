...
var { KeyModelListing, DeleteVehicleKeyAPI, alldeleteUser } = require("../Api/ApiRoutes");
...
function VehicleModelListing() {
  ...
  React.useEffect(() => {
    ...
    Helper.overlay(true);
    setloader(true);
    apiRequest(KeyModelListing.method, KeyModelListing.url)
      .then((response) => {
  ...
  const deleteRecord = (e) => {
    ...
    // );
    // if (delete_property) {
      apiRequest(DeleteVehicleKeyAPI.method, DeleteVehicleKeyAPI.url + `${currentId}`)
        .then((response) => {
          let data = response.data;
          if (response.data.code == 200) {
            //setrecords(response.data.data);
  ...
  const handlePageChange = (pageNumber) => {
    ...
    apiRequest(KeyModelListing.method, KeyModelListing.url + `?page=${pageNumber}`)
      .then((response) => {
        let data = response.data;
        if (response.data.code == 200) {
          setrecords(response.data.data);
