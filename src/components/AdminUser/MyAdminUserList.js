...
var { MyOperatorListAPI,deleteAdmin,alldeleteAdmin } = require("../Api/ApiRoutes");
var { apiRequest } = require("../Api/Service");

function MyAdminUserList() {
  ...
  React.useEffect(() => {
    ...
    Helper.overlay(true);
    setloader(true);
    apiRequest(MyOperatorListAPI.method, MyOperatorListAPI.url)
      .then((response) => {
 ...
 function handleMainCheckBox(e) {
  ...
  const handlePageChange = (pageNumber) => {
    ...
    apiRequest(MyOperatorListAPI.method, MyOperatorListAPI.url + `?page=${pageNumber}`)
      .then((response) => {
        let data = response.data;
        if (response.data.code === 200) {
          setrecords(response.data.data);
...
export default MyAdminUserList;
