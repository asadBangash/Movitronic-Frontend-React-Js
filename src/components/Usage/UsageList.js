...
var { UsageListAPI,deleteUsage, alldeleteUsage  } = require("../Api/ApiRoutes");
...
function UsageList() {
  ...
  React.useEffect(() => {
    if(Helper.getPermissions("usage-read",permissions && permissions,"admin") || (user.type === 'superadmin')){
    Helper.overlay(true);
    setloader(true);
    apiRequest(UsageListAPI.method, UsageListAPI.url)
      .then((response) => {
  ...
  const handlePageChange = (pageNumber) => {
    ...
    apiRequest(UsageListAPI.method, UsageListAPI.url + `?page=${pageNumber}`)
      .then((response) => {
        let data = response.data;
        if (response.data.code == 200) {
          setrecords(response.data.data);
  ...
  return (
    <React.Fragment>
      ...
      {/* <div className="wrapper d-flex align-items-stretch">
        ...
        <div id="content">
          <Header />
          {((Helper.getPermissions('usage-read',permissions && permissions ,'admin')) || (Helper.getPermissions('',[], user.type ))) ?
          <div className="p-4">
                      ...
                      <u>{record.id}</u>
                          ...
                          <p class="card-text">
                            <div className="d-flex justify-content-between">
                              <div>
                               Name: <br/> <strong>{record.vehicle && record.vehicle.name}</strong> 
                              </div>
                              <div>
                             Key Name: <br/>    <strong>{record.vehicle && record.vehicle.key_name}</strong>
                              </div>
                            </div>

                            <div className="d-flex justify-content-between">
                              <div>
                                License: <br/> <strong>{record.vehicle && record.vehicle.license}</strong>
                              </div>
                              <div>
                             Tags:   <br/>   <strong> {record.vehicle && record.vehicle.tags}</strong>
                              </div>
                            </div>

                            Current Usage: <br/>  <strong>{record.vehicle.usages && record.vehicle.usages.current_usage.id}</strong>
                          ...
                          <div className="d-flex justify-content-between">
                              <div>
                                <strong className="btn-success">Start:</strong> <br/>  14:34 12.12.2021
                              </div>
                              <div>
                                <strong>Pick-up:</strong> <br/>  {record.from_latitude}
                              </div>
                            </div>
                            ...
                            <div className="d-flex justify-content-between">
                              ...
                              <div>
                                <strong>Drop-off from:</strong> <br/>  {record.to_latitude}
                              </div>
                            </div>
                          </p>
