...
var { AddWhitelabelListing, DeleteWhitelabelListing, AllDeleteWhitelabelListing } = require("../Api/ApiRoutes");
...
function WhiteLabelList() {
  ...
  function getcheckbox() {
    ...
    if (delete_property) {
      apiRequest(AllDeleteWhitelabelListing.method, AllDeleteWhitelabelListing.url, params)
        .then((response) => {
...
function handleMainCheckBox(e) {
  ...
  const deleteRecord = (e) => {
    ...
    // );
    // if (delete_property) {
      apiRequest(DeleteWhitelabelListing.method, DeleteWhitelabelListing.url + `${currentId}`)
        .then((response) => {
          let data = response.data;
          if (response.data.code == 200) {
            //setrecords(response.data.data);
  ...
  const handlePageChange = (pageNumber) => {
    ...
    apiRequest(AddWhitelabelListing.method, AddWhitelabelListing.url + `?page=${pageNumber}`)
      .then((response) => {
        let data = response.data;
        if (response.data.code == 200) {
          setrecords(response.data.data);
            ...
            <div className="card card-shadow">
                        ...
                        <div className="card-body">
                          ...
                          <p className="card-text">
                            ...
                            <div className="d-flex justify-content-between">
                              <div>
                            TelePhone: <br/><strong>{record.contact_no}</strong> 
                              </div>
                              <div>
                              color: <br/><strong>{record.color}</strong> 
                              </div>
                            </div>
