...
var { DeviceListAPI, SoftwareDeleteAPI, alldeleteUser,DeviceDeleteAPI } = require("../Api/ApiRoutes");
var { apiRequest } = require("../Api/Service");

function DevicesCodeList() {
  ...
  React.useEffect(() => {
    if( Helper.getPermissions("serial-read",permissions && permissions,"admin") || (user.type === 'superadmin')){
    Helper.overlay(true);
    setloader(true);
    apiRequest(DeviceListAPI.method, DeviceListAPI.url)
      .then((response) => {
  ...
  const deleteRecord = (e) => {
    ...
    // );
    // if (delete_property) {
      apiRequest(DeviceDeleteAPI.method, DeviceDeleteAPI.url + `${currentId}`)
        .then((response) => {
          let data = response.data;
          if (response.data.code == 200) {
            //setrecords(response.data.data);
  ...
  const handlePageChange = (pageNumber) => {
    ...
    apiRequest(DeviceListAPI.method, DeviceListAPI.url + `?page=${pageNumber}`)
      .then((response) => {
        let data = response.data;
        if (response.data.code == 200) {
          setrecords(response.data.data);
  ...
  return (
    <React.Fragment>
      ...
      {/* <div className="wrapper d-flex align-items-stretch">
        ...
        <div id="content">
          <Header />
         {((Helper.getPermissions('serial-read',permissions && permissions ,'admin')) || (Helper.getPermissions('',[], user.type ))) ?
            <div className="p-4">
            ...
            <i className="fa fa-cog fa-3x text-center text-primary" aria-hidden="true"></i>
                        <div className="card-body">
                          ...
                          <p className="card-text">
                            <div className="d-flex justify-content-between">
                              <div>
                                <strong>Device Product:</strong> <br/> {record.device}
                              </div>
                              <div>
                                <strong>Hardware:</strong> <br/>  {record.hardware.version}
                              </div>
                            </div>

                            <div className="d-flex justify-content-between">
                              <div>
                                <strong>Date:</strong> <br/>  {record.manufacturing_date}
                              </div>
                              <div>
                                <strong>Server FallBack Path:</strong> <br/> {record.server_fallback_path}
                              </div>
                            </div>

                            <div className="d-flex justify-content-between">
                              <div>
                                <strong>Server Path:</strong><br/> {record.server_path}
                              </div>
               ...
               <button
                        ...
                        Helper.getPermissions("", [], user.type)) && (
                          ...
                          className=""
                          exact
                          to={`/admin/edit-device-code/${record.id}`}
                        >
...
export default DevicesCodeList;
