...
var { CurrentUserAPI, deleteUser, alldeleteUser } = require("../Api/ApiRoutes");
...
function CurrentUserList() {
  ...
  React.useEffect(() => {
    ...
    Helper.overlay(true);
    setloader(true);
    apiRequest(CurrentUserAPI.method, CurrentUserAPI.url)
      .then((response) => {
  ...
  const handlePageChange = (pageNumber) => {
    ...
    apiRequest(CurrentUserAPI.method, CurrentUserAPI.url + `?page=${pageNumber}`)
      .then((response) => {
        let data = response.data;
        if (response.data.code == 200) {
          setrecords(response.data.data);
