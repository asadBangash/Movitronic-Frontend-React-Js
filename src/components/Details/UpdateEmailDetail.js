...
function UpdateEmailDetail() {
  ...
  const handleSubmit = (values) => {
    ...
    apiRequest(UpdateEmailApI.method, UpdateEmailApI.url, form_data)
      .then((response) => {
        if (response.data.code == 200) {
          ...
          setTimeout(function(){ 
            history.push({pathname:'/admin/verify-phone',
            state:{
              user:values.email
            }})
             }, 1000);
  ...
  return (
    <React.Fragment>
      {/* <div className="wrapper d-flex align-items-stretch">
        ...
        <Header />
        <div className="p-4">
            <h4 className="mb-3">User Email Update</h4>
          <Formik 
...
export default UpdateEmailDetail;
